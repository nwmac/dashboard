...
export function compatibleVersionsFor(versions, os) {
...
export function filterAndArrangeCharts(charts, {
  ...
  const out = charts.filter((c) => {
    ...
    if ( isWindows && compatibleVersionsFor(chartVersions, 'windows').length <= 0) {
      // There's no versions compatible with Windows
      return false;
    } else if ( !isWindows && compatibleVersionsFor(chartVersions, 'linux').length <= 0) {
