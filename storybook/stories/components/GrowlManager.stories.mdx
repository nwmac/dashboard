...
import growl from '@/storybook/.storybook/store/growl';
...
export const Template = () => ({
  ...
  store: {...store}.registerModule('growl', growl),
  template:   '<GrowlManager />',
})

# GrowlManager

The GrowlManager component is used to display asynchronous responses of specific actions overlaid on the top right of the window. The information shown in each message includes the severity, a summary, and any details.
...
Messages with different levels of severity can be displayed in the GrowlManager component by dispatching actions to the `growl` store.
